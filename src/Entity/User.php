...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
     ...
     */
    #[ORM\OneToMany(targetEntity: StreamOfTMWR::class, mappedBy: 'user', orphanRemoval: true)]
    ...
    public function removeStreamOfTMWR(StreamOfTMWR $streamOfTMWR): static
    {
        if ($this->streamOfTMWRs->removeElement($streamOfTMWR)) {
            // set the owning side to null (unless already changed)
            if ($streamOfTMWR->getUser() === $this) {
                $streamOfTMWR->setUser(null);
            }
        }
